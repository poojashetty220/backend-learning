...
function signRequest(transaction) {
  const apiBase = 'https://2lkwyi8sxi.execute-api.ap-south-1.amazonaws.com/dev';
...
hooks.beforeEach((transaction, done) => {
  ...
  // POST /orders
  if (transaction.request.uri === '/orders' && transaction.request.method === 'POST') {
    if (transaction.expected.statusCode === '400') {
      transaction.request.body = JSON.stringify({});
    }
    if (transaction.expected.statusCode === '201') {
      transaction.request.body = JSON.stringify({ 
        order_number: `ORD-${generateRandomString(6)}`,
        total_amount: '250.75',
        user_id: '64a8f9b3d5e2a7c9b8f4a2d1',
        user_name: 'John Doe'
      });
    }
  }
